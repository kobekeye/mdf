// SVG header reconciliation for typst.ts incremental SVG patches.
//
// `patchRoot()` from typst.ts walks both trees expecting children in the same
// order. The `<style>` and `<defs class="…">` resource headers can drift in
// count or order across renders (e.g. fonts get added, clip-path subtree
// pruned), so we deduplicate + reorder both sides before patching, and we
// insert placeholders into `next` for headers that exist only in `prev`. That
// keeps the patch indices aligned and avoids node-type clashes.
...
/**
 * Replace the container's SVG with the patched-or-fresh version of `svgText`.
 * If the container already holds an SVG, the new one is patched into it via
 * typst.ts's `patchRoot`. Otherwise the inner HTML is replaced wholesale.
 */
